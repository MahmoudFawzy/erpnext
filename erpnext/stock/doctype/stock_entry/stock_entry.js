...
frappe.ui.form.on('Stock Entry', {
	setup: function (frm) {
		...
		frappe.db.get_value('Stock Settings', {
			name: 'Stock Settings'
		}, 'sample_retention_warehouse', (r) => {
...
erpnext.stock.StockEntry = erpnext.stock.StockController.extend({
	...
	set_default_account: function (callback) {
		...
		if (frm.doc.purpose == "Material Issue") {
			acc = "default_expense_account";
		} else if (frm.doc.purpose == "Material Receipt") {
			acc = "stock_received_but_not_billed";
		} else {
			acc = "stock_adjustment_account";
		}
...
$.extend(cur_frm.cscript, new erpnext.stock.StockEntry({
	frm: cur_frm
}));