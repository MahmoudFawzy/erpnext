...
$.extend(cur_frm.cscript, new erpnext.hr.ExpenseClaimController({
	frm: cur_frm
}));
...
frappe.ui.form.on("Expense Claim Detail", {
	expense_type: function (frm, cdt, cdn) {
		...
		frappe.call({
			...
			callback: function (r) {
				...
				if (r.message.allow_vat) {
					frappe.model.set_value(cdt, cdn, 'vat', 5);
				}
			}
		});
	...
	amount: function (frm, cdt, cdn) {
		...
		var val = child.claim_amount;
		if (child.vat > 0) {
			val = val * (child.vat / 100) + val;
		}
		frappe.model.set_value(cdt, cdn, 'sanctioned_amount', val);
